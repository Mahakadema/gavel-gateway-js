...
module.exports.fetchRaw = function (options) {
    ...
    if (parsedOptions.allowCache && parsedOptions.interpret) {
        ...
        if (entry && entry.timestamp + entry.cacheFor > now) {
            try {
                checkMultiChoice(entry.raw.body, entry.raw.status);
            } catch (e) {
    ...
    request.promise = new Promise(async (resolve, reject) => {
        ...
        if (parsedOptions.interpret) {
            try {
                checkMultiChoice(newCacheEntry.body, newCacheEntry.status)
            } catch (e) {
                reject(e);
                return;
            }
...
module.exports.fetchLeaderboardTypes = async function (options) {
    ...
    const response = await module.exports.fetchRaw(parsedOptions);
    return new List(response.body.slice(), {
        requestedAt: response.requestedAt,
        respondedAt: response.respondedAt,
        receivedAt: response.receivedAt,
        timestamp: response.dataTimestamp,
        apiVersion: semVerFromHeader(response.headers["version"]),
        libVersion: "1.0.0",
...
module.exports.fetchPlayerLeaderboard = async function (options) {
    ...
    const dataAPI = await module.exports.fetchRaw(parsedOptions);
    const arr = dataAPI.body.data.map(v => new LeaderboardPlayer(v));
...
module.exports.fetchPlayer = async function (options) {
    ...
    if (parsedOptions.forceUUIDLookup && /^[A-Za-z0-9_]{1,16}$/.test(parsedOptions.player)) {
        const uuid = await module.exports.fetchPlayerUUID(parsedOptions.player);
        if (!uuid)
            return null;
            const searchResult = await module.exports.fetchNames(parsedOptions.player);
            const uuid = (searchResult.players.find(v => v.name === parsedOptions.player) ?? searchResult.players.find(v => v.name.toLowerCase() === parsedOptions.player.toLowerCase()))?.uuid;
            if (!uuid)
                return null;
            parsedOptions.route = sanitizeURL(config.routes.PLAYER.url.replace("%v", uuid));
...
module.exports.fetchPlayerCharacterAbilityTree = async function (options) {
    ...
    parsedOptions.route = sanitizeURL(config.routes.PLAYER_CHARACTER_AT.url.replace("%v", parsedOptions.player).replace("%w", parsedOptions.character));
    parsedOptions.routeVersion = config.routes.PLAYER_CHARACTER_AT.version;
    parsedOptions.cacheFor ??= config.routes.PLAYER_CHARACTER_AT.cacheTime;
    const [characterTree, abilityTree] = await Promise.all([
        module.exports.fetchRaw(parsedOptions),
        module.exports.fetchAbilityTree(options)
    ]);
    ...
    const selected = await checkMultiChoice(dataAPI.body, dataAPI.status, parsedOptions, (k, v) => ({
        ...
        rank: {
            ...
            donatorRank: v.supportRank,
            textColor: v.legacyRankColour?.main ?? null,
            textColor: v.legacyRankColour?.sub ?? null,
            badgeUrl: CDN_URL + v.rankBadge
        },
        fetch: (options = {}) => module.exports.fetchPlayerCharacterAbilityTree({ ...options, player: k })
    }));
    if (selected !== undefined)
        return selected;

    return characterTree.body === null ? null : new PlayerCharacterAbilityTree(characterTree, {
        source: parsedOptions.route,
...
module.exports.fetchAbilityTree = async function (options) {
    const parsedOptions = parseAbilityTreeRequestOptions(options);
    const mapOptions = {
        ...parsedOptions,
    ...
    return new AbilityTree(map, tree, { source: mapOptions.route })
}

// Fetches a guild
module.exports.fetchGuild = async function (options) {
    const parsedOptions = parseGuildRequestOptions(options);
    parsedOptions.route = sanitizeURL(config.routes.GUILD.url.replace("%v", parsedOptions.path + parsedOptions.guild));
    parsedOptions.routeVersion = config.routes.GUILD.version;
    parsedOptions.cacheFor ??= config.routes.GUILD.cacheTime;
    const [guildStats, territories] = parsedOptions.fetchAdditionalStats ?
        await Promise.all([
            module.exports.fetchRaw(parsedOptions),
    ...
    return guildStats.body === null ? null : new Guild(guildStats, {
...
module.exports.fetchGuildList = async function (options) {
    ...
    const guildList = await module.exports.fetchRaw(parsedOptions);
    return new List(guildList.body.map(v => new GuildListItem(v)), {
        requestedAt: guildList.requestedAt,
        respondedAt: guildList.respondedAt,
        receivedAt: guildList.receivedAt,
        timestamp: guildList.dataTimestamp,
        apiVersion: semVerFromHeader(guildList.headers["version"]),
        libVersion: "3.0.0",
        source: parsedOptions.route
    });
};
...
module.exports.fetchTerritoryList = async function (options) {
    ...
    const territoryList = await module.exports.fetchRaw(parsedOptions);
    return new List(getTerritoryList(territoryList.body), {
        requestedAt: territoryList.requestedAt,
        respondedAt: territoryList.respondedAt,
        receivedAt: territoryList.receivedAt,
        timestamp: territoryList.dataTimestamp,
        apiVersion: semVerFromHeader(territoryList.headers["version"]),
        libVersion: "1.2.0",
        source: parsedOptions.route
    });
};
...
module.exports.fetchItems = async function (options) {
    ...
    let arr = Object.getOwnPropertyNames(itemList.body).map(v => ({ k: v, v: itemList.body[v] }))
        .filter(({ v }) => v.type || v.accessoryType)
...
const {
    ...
    parseAbilityTreeRequestOptions,
} = require("./util.js");
