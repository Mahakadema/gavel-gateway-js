...
module.exports.fetchRaw = function (options) {
    ...
    let queued = false;
    if (chosenRatelimit.remaining <= 0 || ratelimits.GLOBAL.ongoingRequestsAmount > 25) { // ISPs often block incoming requests if they reach a high amount per second, throttle to prevent that
    ...
    request.data = (async () => {
        if (queued) {
            ...
            while (ratelimits.GLOBAL.ongoingRequestsAmount > 25)
                await sleep(500);
            // delay request in long increments until ratelimit expired, then do fast increments
            if (request.priority) {
                while (chosenRatelimit.remaining <= 0 || chosenRatelimit.priorityQueue[0].id !== request.id) {
                    await sleep(10);
                ...
                while (chosenRatelimit.remaining <= 0 || chosenRatelimit.priorityQueue.length > 0 || chosenRatelimit.queue[0].id !== request.id) {
                    await sleep(10);
                    if (chosenRatelimit.remaining <= 0) {
                        await sleep(500);
                        if (chosenRatelimit.reset < Date.now()) {
