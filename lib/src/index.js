...
// remove all entries from cache (here because config.js has a circular dependency to this)
...
module.exports.fetchRaw = function (options = {}) {
    ...
    if (parsedOptions.allowStacking) {
        ...
        for (const key of config.apiKeys) {
            let requestIndex = ratelimits[key].queue.findIndex(v => v.route === parsedOptions.route);
            if (requestIndex >= 0) {
                ...
                const queuedRequest = ratelimits[key].priorityQueue.find(v => v.route === parsedOptions.route);
                if (queuedRequest) {
                    return config.reuseJson ? queuedRequest.data : queuedRequest.data.then(v => JSON.parse(JSON.stringify(v)));
                }
            }
...
module.exports.fetchGuildList = async function (options) {
    try {
        ...
        const dataAPI = await module.exports.fetchRaw(parsedOptions);
        const data = new List(dataAPI.guilds, { requestTimestamp: now, timestamp: dataAPI.request.timestamp * 1000 + timeDisparity });
        return data;
    } catch (e) {
        throwErr(e);
    }
...
module.exports.fetchRecipeList = async function (options) {
    try {
        ...
        const dataAPI = await module.exports.fetchRaw(parsedOptions);
        const data = new List(dataAPI.data, { requestTimestamp: now, timestamp: dataAPI.timestamp + timeDisparity });
        return data;
    } catch (e) {
        throwErr(e);
    }
