...
module.exports.set = function (configOptions) {
    ...
    // sync time
    if (configOptions.syncTime !== undefined && configOptions.syncTime !== null) {
        if (typeof configOptions.syncTime === "boolean") {
            config.syncTime = configOptions.syncTime;
        } else {
            throw new TypeError(`Expected 'syncTime' to be a boolean, but received ${typeof configOptions.syncTime} instead`);
        }
    }
    // cache times
