...
module.exports.set = function (configOptions) {
    ...
    if (configOptions.defaultCacheTimes) {
        for (const route in config.routes) {
            if (config.routes.hasOwnProperty(route) && configOptions.defaultCacheTimes.hasOwnProperty(route) && configOptions.defaultCacheTimes[route] !== null && configOptions.defaultCacheTimes[route] !== undefined) {
