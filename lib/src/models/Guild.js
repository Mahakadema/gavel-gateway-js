...
class Guild extends BaseAPIObject {
    constructor(json, params) {
        ...
        if (this.xpFriendly.isSafe) {
            this.xpFriendly.maxErrorLower = 1 + guildLevels[a.level].maxError;
            this.xpFriendly.maxErrorUpper = 1 + guildLevels[a.level].maxError + 0.1 / a.xp; // natural error + error due to xp being truncated
            this.xpFriendly.xpPct = a.xp * guildLevels[a.level].preGavelReborn / guildLevels[a.level].postGavelReborn / 100;
            this.xpFriendly.xpRaw = Math.round(a.xp * guildLevels[a.level].preGavelReborn / 100);
