...
class Item {
    constructor(v) {
        ...
        this.requirements = {
            level: v.level,
            quest: v.quest,
            class: v.classRequirement?.toUpperCase() ?? ["WARRIOR", "ARCHER", "MAGE", "ASSASSIN", "SHAMAN"][["SPEAR", "BOW", "WAND", "DAGGER", "RELIK"].indexOf(this.type)] ?? null,
            strength: v.strength,
        ...
        for (const id of identifications) {
            if (v[id.itemApiName]) { // ID !== 0 (and also in case a new version of the API removes an ID)
                this.identifications.push({
                    ...
                    // if ID static or item prerolled, fixed values, else if less than 0, swap min and max, if inverted, change min roll multiplier
                    min: id.static || this.identified ? v[id.itemApiName] : v[id.itemApiName] > 0 ? id.inverted ? Math.max(1, Math.round(v[id.itemApiName] * 0.7)) : Math.max(1, Math.round(v[id.itemApiName] * 0.3)) : Math.round(v[id.itemApiName] * 1.3),
                    max: id.static || this.identified ? v[id.itemApiName] : v[id.itemApiName] > 0 ? Math.round(v[id.itemApiName] * 1.3) : id.inverted ? Math.min(-1, Math.round(v[id.itemApiName] * 0.3)) : Math.min(-1, Math.round(v[id.itemApiName] * 0.7)),
                });
            }
        }
        this.majorIds = [];
